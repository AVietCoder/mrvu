...
th{padding:10px 8px;font-size:12px;font-weight:600}
...
<div class="info-grid">
  <div><div class="info-label">Khách hàng</div><div class="info-value">${custName ?? "Khách lẻ"}${custPhone ? " — " + custPhone : ""}</div></div>
  <div><div class="info-label">Chi nhánh</div><div class="info-value">${branchName ?? "—"}</div></div>
  <div><div class="info-label">Nhân viên</div><div class="info-value">${empName ?? "—"}</div></div>
  <div><div class="info-label">Hình thức TT</div><div class="info-value">${pmLabel}</div></div>
  ${custAddress ? `<div style="grid-column:span 2"><div class="info-label">Địa chỉ</div><div class="info-value">${custAddress}</div></div>` : ""}
</div>
...
<div class="total-section"><div class="total-box">
  <div class="total-row"><span>Tạm tính</span><span>${moneyFmt(order.subtotal ?? 0)}</span></div>
  ...
  <div class="total-row"><span>Tổng cộng</span><span style="font-weight:700">${moneyFmt(order.total ?? 0)}</span></div>
  ...
  <div class="total-row grand"><span>Khách cần trả</span><span>${moneyFmt(Math.max(0, (order.total ?? 0) - (order.deposit ?? 0) - (order.paid ?? 0)))}</span></div>
</div></div>
...
<div class="checklist">
  ...
  <div style="display:flex;justify-content:space-between;margin-top:10px;font-size:11px;color:#6b7280">
    ...
    <span>Họ và tên: _________________ &nbsp;&nbsp; Chữ ký: ______________</span>
  </div>
</div>
...
function printOrderSlip({
  ...
  const fakeOrder = {
    code: undefined, created_at: new Date().toISOString(), status,
    payment_method: paymentMethod, subtotal, discount: discountAmt,
    vat_amount: includeVat ? vatAmt : 0, total, deposit, paid: 0, note,
  };

  const pw = window.open("", "_blank");
  if (!pw) return;
  pw.document.write(buildOrderPrintHtml({
    order: fakeOrder,
    custName: custObj?.name ?? "Khách lẻ",
