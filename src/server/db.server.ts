...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS products (
    ...
    brand TEXT, power TEXT, color TEXT, blade_size TEXT,
  ...
  -- Users = nhân viên (đồng bộ một bảng)
...
function seedIfEmpty() {
  ...
  const insP = db.prepare(`INSERT INTO products (id,sku,name,category_id,brand,power,color,blade_size,cost_price,sale_price,min_stock) VALUES (?,?,?,?,?,?,?,?,?,?,?)`);
  ...
  insP.run("p4","QT-CN-004","Quạt trần CN HVLS 12ft","c3","MaxAir","550W","Xám",'144"',18500000,24900000,2);
  ...
  db.prepare("INSERT INTO user_branches (user_id,branch_id) VALUES (?,?)").run("u2","b1");
  db.prepare("INSERT INTO user_branches (user_id,branch_id) VALUES (?,?)").run("u3","b2");

  // Cấp quyền mẫu cho u2
  db.prepare("INSERT INTO user_permissions (user_id,permission) VALUES (?,?)").run("u2","create_order");
  ...
  db.prepare(`INSERT INTO stock_movements (id,type,product_id,to_branch,qty,unit_cost,note) VALUES (?,?,?,?,?,?,?)`)
